...
def main():
    ...
    logger = logging.getLogger("Region4_RLSN")

    logger.info("Starting RLSN Extraction for Region 4...")
    
    # 1. Load data
    loader = AISDataLoader(config.DATA_FILE_PATH)
    ais_df = loader.load_data()
    
    # 2. Load NLSN results (Best Gamma 0.001)
    nlsn_best_dir = os.path.join(config.NLSN_RESULTS_DIR, "gamma_0p001")
    ...
    logger.info("Region 4 RLSN Extraction completed. Generated %d routes.", len(rlsn_routes))
