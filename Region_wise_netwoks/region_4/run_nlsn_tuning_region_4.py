...
def main():
    parser = argparse.ArgumentParser(description="Region 4 NLSN gamma sweep.")
    ...
    logger = logging.getLogger("Region4-NLSN-GammaSweep")

    # 1. Load best PLSN params
    plsn_summary = args.plsn_summary
    if not plsn_summary:
        tuning_dir = config.TUNING_DIR
        summaries = [f for f in os.listdir(tuning_dir) if f.startswith("region4_plsn_tuning_") and f.endswith("_summary.json")]
        if summaries:
            summaries.sort()
    ...
    # 4. Run Sweep
    logger.info("Starting NLSN Gamma Sweep for Region 4...")
    ...
    print(f"\n[OK] Region 4 NLSN Tuning completed.")
