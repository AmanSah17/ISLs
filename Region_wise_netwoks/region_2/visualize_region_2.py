...
def main():
    logger.info("Generating Final Integrated Dashboard for Region 2...")
    
    output_html = os.path.join(REGION_DIR, "integrated_dashboard_region_2.html")
    ...
    if all(os.path.exists(p) for p in [routes_path, bounds_path]):
        visualizer.add_rlsn_layer("Region 2 RLSN", json.load(open(routes_path)), json.load(open(bounds_path)), batch="Route Corridors")
    
    visualizer.generate_dashboard()
    logger.info("SUCCESS: Region 2 Integrated Dashboard generated at %s", output_html)
