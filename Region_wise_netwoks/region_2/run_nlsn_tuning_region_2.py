...
def main():
    parser = argparse.ArgumentParser(description="Region 2 NLSN gamma sweep.")
    ...
    logger = logging.getLogger("Region2-NLSN-GammaSweep")

    # 1. Load best PLSN params
    plsn_summary = args.plsn_summary
    if not plsn_summary:
        tuning_dir = config.TUNING_DIR
        summaries = [f for f in os.listdir(tuning_dir) if f.startswith("region2_plsn_tuning_") and f.endswith("_summary.json")]
        if summaries:
            summaries.sort()
    ...
    # Correct columns: BaseDateTime, LAT, LON
    ...
    print(f"\n[OK] Region 2 NLSN Tuning completed.")
