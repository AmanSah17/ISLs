...
def main():
    parser = argparse.ArgumentParser(description="Region 1 NLSN gamma sweep.")
    ...
    logger = logging.getLogger("Region1-NLSN-GammaSweep")

    # 1. Load best PLSN params
    # We try to auto-find the latest plsn summary if not provided
    plsn_summary = args.plsn_summary
    if not plsn_summary:
        tuning_dir = config.TUNING_DIR
        summaries = [f for f in os.listdir(tuning_dir) if f.startswith("region1_plsn_tuning_") and f.endswith("_summary.json")]
        if summaries:
            summaries.sort()
    ...
    # Note: run_nlsn_gamma_sweep uses its own memory-safe loader, but for Region 1 we can use pandas directly
    ...
    # 4. Run Sweep
    logger.info("Starting NLSN Gamma Sweep...")
    ...
    print(f"\n[OK] Region 1 NLSN Tuning completed.")
