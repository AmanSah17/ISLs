...
def main():
    parser = argparse.ArgumentParser(description="Region 5 NLSN gamma sweep.")
    ...
    logger = logger = logging.getLogger("Region5-NLSN-GammaSweep")

    # 1. Load best PLSN params
    plsn_summary = args.plsn_summary
    if not plsn_summary:
        tuning_dir = config.TUNING_DIR
        summaries = [f for f in os.listdir(tuning_dir) if f.startswith("region5_plsn_tuning_") and f.endswith("_summary.json")]
        if summaries:
            summaries.sort()
    ...
    # 4. Run Sweep
    logger.info("Starting NLSN Gamma Sweep for Region 5...")
    ...
    print(f"\n[OK] Region 5 NLSN Tuning completed.")
